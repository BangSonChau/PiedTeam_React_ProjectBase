import type { PaginatedResponse, SelectOption } from "@/shared/type";
  ...
  return {
    getAll:
      ...
      (async (params?: TFilterParams) => {
        return api.get<PaginatedResponse<TEntity>>(endpoint, { params }) as unknown as Promise<PaginatedResponse<TEntity>>
      }),

    getById:
      config.getById ??
      (async (id: string | number) => {
        return await api.get<TEntity>(`${endpoint}/${id}`) as unknown as Promise<TEntity>
      }),

    create:
      config.create ??
      (async (data: TCreateDto) => {
        return await api.post<TEntity>(endpoint, { data }) as unknown as Promise<TEntity>
      }),

    update:
      config.update ??
      (async (id: string | number, data: TUpdateDto) => {
        return await api.put(`${endpoint}/${id}`, { data }) as unknown as Promise<TEntity>
      }),
    ...
    getSelectOption:
      ...
      (async () => {
        return await api.get<SelectOption[]>(`${endpoint}/select`) as unknown as Promise<SelectOption[]>
      })
  }
}
